...
class ReactivePathFollower:
    ...
    def next_goal(self, x):
        ...
        # as in pure-pursuit path following.
        #
        ...
        # progress). The previous implementation picked the GLOBAL
        # argmin of |dist - rho| over the full lap, which could lock onto
        # the second (degenerate, near-zero-progress) solution whenever it
        # happened to match rho at least as well as the first -- causing
        # the follower to stall indefinitely a few percent short of a full
        # lap instead of advancing around the path. Taking the first
        # forward crossing removes this ambiguity by construction.
