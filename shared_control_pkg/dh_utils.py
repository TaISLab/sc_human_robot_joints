...
CORRECTION (post-review): row 4 previously carried the forearm length
l2 as the DH offset d_4 with alpha_4 = 0, i.e. Rot_z(theta_4) followed
by a translation ALONG the same z-axis the rotation happens about.
Since rotation about an axis and translation along that same axis
commute, that parameterization made the wrist POSITION exactly
independent of q4 for every posture (elbow flexion only ever changed
orientation, never position) -- discovered while diagnosing why the
closed-loop simulator's virtual users could not complete a full circle
trace. It is fixed here using the standard anthropomorphic convention:
the forearm length enters as the link length a_4 (offset along the
rotated frame's own x-axis), so that q4 now properly changes the
wrist's position relative to the elbow, as a real elbow flexion does.
"""
...
def cartesian_to_human_joint_velocity(q, l1, l2, v_cartesian, damping=1e-3, J=None,
    ...
    it only decides where the redundant degree of freedom is spent. Used
    by sim/virtual_human_ik.py and sim/closed_loop_sim.py to drive the
    OFFLINE SYNTHETIC human model in a more physiologically plausible way
    (a real arm does not sit at an arbitrary point of its self-motion
    manifold just because a fixed IK seed happened to put it there); NOT
