...
Every candidate command k in {h, r} (human, robot) receives its own
performance eta_k in [0, 1], obtained as a weighted average of the
four partial factors (Eq. 6 in the reactive-SC framework, generalized
from m=2 to m=4):
...
def manipulability_factor(J_current, J_predicted, Cm=1.0):
    ...
    configuration q + qdot_candidate * dt (obtained externally, e.g.
    via PyKDL/URDF, see shared_control_node.RobotModel).
    """
...
# ---------------------------------------------------------------------
# Weighted combination (Eq. 6, generalized to m=4)
# ---------------------------------------------------------------------
