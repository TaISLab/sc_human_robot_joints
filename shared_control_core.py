...
the SAME code path used by:

  * shared_control_node.py (real robot, ROS I/O), and
  * sim/closed_loop_sim.py (offline Monte-Carlo simulation),

so that anything validated in simulation is guaranteed to behave
identically once deployed, and any tuning done in simulation
(performance weights, C constants, thresholds) transfers directly.
"""
...
class SharedControlCore(object):
    ...
    def step(self, v_h, v_r, tangent, q_human=None, l1=None, l2=None,
        ...
        # per-cycle cost by half (see sim/benchmark.py).
        J_arm_human = None
        if "joint_safety" in active_factors and q_human is not None:
